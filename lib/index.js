	...
	if (SUPPORTED_INPUT_TYPES.includes(inputFileExtension)) {
		fileLoader(input)
			.then((gltf) => {
				...
				gltfValidator.validateBytes(new Uint8Array(Buffer.from(JSON.stringify(preProcessed))))
					.then((report) => {
					...
					.then((result) => {
						...
						if (postProcessed.separateResources) {
							Object.keys(postProcessed.separateResources).forEach((resource) => {
								fs.writeFile(`${getFilePath(output)}${resource}`, postProcessed.separateResources[resource], (error) => {
									if (!error) {
